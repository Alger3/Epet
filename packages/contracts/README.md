...
- `.epet` 的 `schema_version` 控制结构兼容，`package_version` 控制单个宠物包修订，`min_runtime_version` 控制加载门槛。
- 生成类型只改来源契约后重新生成，禁止人工编辑。

## 语义校验

Schema 只能检查结构。加载器还必须检查：manifest 动作帧与时长数量相同、所有帧存在、Atlas 矩形不越界、文件清单与压缩包完全一致、哈希/大小匹配、路径安全、许可证存在和资源上限。
