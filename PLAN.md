...
- 本地开发 API、基础设施、确定性 Mock Worker 和桌面安装闭环已经可用；Mock 仍只验证管线，不是 Q 版 AI。多帧骨骼动画、真实模型 Provider、Gate B-P/B-H 和公开发布能力仍属于后续步骤。
...
6. AI 生成由独立 Worker Provider 完成；本地模式优先按 CUDA、OpenVINO GPU、OpenVINO CPU 自动选择，后续可把同一任务契约部署到云端 GPU。桌面 UI 只负责照片预处理、上传、任务展示、资源安装和角色运行。
...
| 阶段 5：生成服务基础 | 第 4–8 周 | 共享任务、存储、多帧骨骼动画基线和多硬件 Worker Provider 可用 |
...
> 当前实现进度（2026-07-27）：阶段 4 的桌面端闭环已完成。工坊具备首页、我的角色、创建角色、运行设置和隐私页；SQLite v8 持久化不可变主体类型、版本化人物授权、照片槽位、任务快照版本、错误和重试状态。照片在 WebView 应用方向与裁剪后重新编码，再由 Rust 限额解码、统一缩放、去元数据、复解码和 SHA-256 校验；草稿取消/删除和中断恢复会清理本地副本。“我的角色”支持缩略图、缺失资源提示、使用、隐藏、重命名、更新、旧版本保留/回滚、重新下载和可回滚删除，已安装 `.epet` 会直接接入桌宠运行时。步骤 4.3 已接入本地 FastAPI、SSE/轮询、Mock 产物下载、SHA-256 校验、自动安装和激活；当前 Mock 产物仍是单帧静态角色，不把它表述为 Q 版 AI 或最终动作预览。
...
>
> 状态：Completed（2026-07-28）。已交付 `.epet` v2 动画元数据、猫咪/人物标准 Rig 与 LayerBundle、确定性离线 Atlas 渲染、八动作多帧产物、距离相位播放、v1 静态包兼容、黄金哈希和本地双主体黑盒闭环。这里的 Mock 只从照片提取稳定配色并驱动原创程序化部件，不代表真实照片已经完成 Q 版重绘；该能力进入步骤 5.5。

要做什么：

- 在接入真实生成模型前，先把当前“一张图片复用所有动作”的 Mock 产物升级为多帧动画角色。
- 建立标准立绘、部件层、模板骨骼、动作 Clip、离线渲染和确定性 Atlas 的统一中间契约。
- 先使用内置原创测试猫咪和人物验证动画，不把模型身份一致性问题与动画系统问题混在同一轮调试。
- 最终客户端仍只加载 `.epet` Sprite Atlas；骨骼、部件层和动画求解只存在于 Worker 制作阶段。

怎么做：

- 定义版本化 `LayerBundle`、`RigDefinition`、`AnimationClip` 和 `RenderProfile` Schema。
- 猫咪首版拆分身体、头、耳朵、眼睛、四肢/爪和 4 段尾巴；人物首版拆分骨盆、躯干、头、头发前后层、眼睛、手臂、腿、手脚和服装前后层。
- 首版使用刚性部件和分段附属骨骼；头发、衣摆、长毛和尾巴使用 2–4 段骨骼产生次级运动，暂不引入通用网格蒙皮。
- 为猫咪和人物分别制作 `idle`、`walk`、`sleep`、`tap`、`drag` 和 `wake`；人物可增加 `wave`。
- 猫咪走路包含四肢交替、身体起伏、头部稳定和尾巴平衡；人物走路包含手脚反向摆动、骨盆升降以及头发/衣摆延迟。
- 睡眠必须包含姿势下沉或收拢、闭眼、呼吸和局部抽动；`zzz` 只作为可关闭的效果层，不能充当睡眠动作本身。
- 走路帧使用累计移动距离而不是墙钟时间驱动，按 `distance / stride_length` 计算相位，避免脚底打滑；其他动作继续按 manifest 帧时长播放。
- 使用固定求值顺序、2 倍超采样、统一脚底锚点、Alpha 边缘扩展和稳定 Atlas 排序生成多帧资源。
- Mock Worker 改为调用动画流水线；相同测试输入、模板、Clip 和种子必须生成字节级一致的 `.epet`。
- 自动 QA 检查部件重组差异、关节裂缝、越界、脚滑、中心抖动、循环接缝、次级运动穿模、透明边缘和包完整性。

实施顺序：

1. 冻结中间 Schema 与两套原创测试 LayerBundle。
2. 实现确定性部件合成、骨骼变换和 Atlas 渲染器。
3. 完成猫咪动作与桌面距离相位同步。
4. 完成人物动作与次级运动。
5. 加入黄金帧、循环、脚滑和黑盒 `.epet` 加载测试。
6. 将现有 Mock Worker 切换为多帧动画产物。
7. 动画基线通过后才开始步骤 5.5 的真实模型 Provider。

验收标准：

- 测试猫咪移动时四肢交替、身体起伏和尾巴摆动，脚步与窗口实际移动距离同步。
- 测试人物移动时手脚交替、身体升降，头发或衣摆存在受限次级运动。
- 关闭 `zzz` 后仍能从闭眼、姿势和呼吸明确识别猫咪与人物处于睡眠状态。
- 八个动作均包含不同的多帧画面，循环无明显闪帧，非循环动作能按 fallback 返回。
- 同一输入和动画版本重复渲染得到相同 Atlas 与 `.epet` SHA-256。
- 最终包通过现有 Rust 校验、自动安装和激活，并继续满足 30MB 包限制。
...
- 用户拖动桌宠到当前显示器工作区的任意边缘并松手时，进入 `perch` 趴伏状态，只露出头和前爪/双手。
- 记录 `edge_dock = left | right | top | bottom`，重启后保留边缘姿态；再次拖离边缘或重置位置时退出。
- 自动移动碰到边缘仍按原逻辑折返，不自动进入趴伏，避免桌宠在无人操作时卡住。
- 睡眠与边缘停靠采用组合状态：`sleep + edge_dock` 派生播放 `perch_sleep`，拖回屏幕中央后继续普通睡眠，不因移动位置而自动唤醒。
- 清醒 `perch` 达到无操作睡眠时间后进入 `perch_sleep`；三击唤醒后返回 `perch`，而不是离开边缘。

怎么做：

- 使用 32 逻辑像素吸附阈值，按显示器 DPI 换算物理像素，并以工作区边界处理任务栏、多显示器和负坐标。
- 拖拽落点选择最近边缘、夹紧窗口并进入 `perch`；开始下一次拖拽时立即清除停靠字段。
- `.epet` 动作集合包含 8 帧循环 `perch` 和 8 帧闭眼呼吸 `perch_sleep`；内置猫咪和 Q 版人物使用专用透明素材，上、左、右边缘由运行时旋转同一姿态。
- SQLite 运行时 Schema 升级到 v9，旧数据库自动迁移；旧 `.epet` 没有 `perch` 时继续回退到 `idle`。

验收标准：

- 四个工作区边缘均能吸附并显示正确方向的趴伏动画，拖离后恢复正常拖拽/落下状态。
- 重启、重置位置、不同 DPI 和负坐标显示器下状态与位置一致。
- 猫咪和人物只显示头与前爪/手，不是整张站立图缩放；生成包和内置角色都包含多帧 `perch`。
- 趴睡状态保持闭眼、伏在爪/手臂上并缓慢呼吸；睡着从边缘拖回中央后仍为 `sleep`。
- Rust 状态/边缘判定、前端资源、Worker 确定性和 `.epet` 兼容测试通过。
...
- 种子由输入照片摘要、主体类型和工作流版本稳定派生；Provider 输出依次进入部件拆分、阶段 5.4 动画渲染、确定性 `.epet` 打包、SHA-256 校验和自动安装链路。
...
- 将真实猫咪标准立绘和部件 Mask 适配到阶段 5.4 已冻结的猫咪 LayerBundle 与骨骼模板。
- 使用共享动画引擎生成待机、走路、睡觉、点击、拖拽和唤醒动作。

怎么做：

- 阶段 5.4 负责动画引擎、Clip 和短毛猫基础模板；本步骤只增加经过评估的长毛猫 B、幼猫 A 参数，狗模板不进入本阶段。
- 将真实拆分结果映射到模板画布、脚底线、身体关键点、旋转中心和碰撞区域；绑定失败返回部件级原因。
- 使用已确认标准立绘作为身份参考，使用姿态图或轮廓作为动作约束。
- 生成模型只在模板无法覆盖的姿势上提供少量关键帧；中间帧统一交给阶段 5.4 的骨骼和受控插值生成。
- 待机动作优先使用呼吸、眨眼、尾巴摆动等局部变化，降低身份漂移。
- 走路动作要求脚底基线和循环首尾一致。
- 无论中间使用骨骼还是网格，最终动作都烘焙成 Sprite 帧，客户端不加载骨骼、模型权重或生成脚本。
- 模板轻动画基线必须已在阶段 5.4 通过；只有身份一致性 Gate B-P 通过后才启用猫咪生成式关键姿态。
...
- 将真实人物标准立绘和部件 Mask 适配到阶段 5.4 已冻结的人物 LayerBundle 与骨骼模板，复用共享引擎生成待机、走路、睡觉、点击、拖拽和唤醒；轻微挥手作为人物可选动作并回退到 `idle`。
