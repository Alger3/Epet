...
>
> 状态：Completed（2026-07-27）。已交付 `.epet` v2 动画元数据、猫咪/人物标准 Rig 与 LayerBundle、确定性离线 Atlas 渲染、七动作多帧产物、距离相位播放、v1 静态包兼容、黄金哈希和本地双主体黑盒闭环。这里的 Mock 只从照片提取稳定配色并驱动原创程序化部件，不代表真实照片已经完成 Q 版重绘；该能力进入步骤 5.5。
...
- 七个动作均包含不同的多帧画面，循环无明显闪帧，非循环动作能按 fallback 返回。
...
- `.epet` 动作集合增加 8 帧循环 `perch`，内置猫咪和 Q 版人物使用专用“露头 + 露手”透明素材；上、左、右边缘由运行时旋转同一姿态。
